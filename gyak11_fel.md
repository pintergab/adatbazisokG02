...
token@cqlsh:kps> insert into szemely(nev, szulev,foglalkozas) values( 'Nagy Ivó' , 2000, 'diak');
token@cqlsh:kps> insert into szemely(nev, szulev,foglalkozas) values( 'Toth Otto' , 1980, 'pincer');
token@cqlsh:kps> select *
       ... from szemely
```
...
```js
 select nev
       ... from szemely
       ... where foglalkozas='lakatos' OR foglalkozas='pincer';


select nev
from szemely
where foglalkozas in('pincer', 'lakatos') // az or nem elfogadott
```
...
```js
 ...
 update szemely
       ... set vegzetsg=vegzettseg + ['gepesz' , 'muszeresz']
